...
I loaded the Quake2.z64 rom to Ghidra and wasted a boatload of time trying to figure out how to sync Ares emulator with it and track the execution to the password generation function. That didn't work out, so I just dumped the full decompilation and fed it to Gemini, asking it to find that ****, and it did.

I also did a ton of black box testing generating valid codes for specific circumstances, which could then be used to both validate the Gemini-generated decryption code and to align it with the game's actual behavior.
